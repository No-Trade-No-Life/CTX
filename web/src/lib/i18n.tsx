import {
  createContext,
  useContext,
  useEffect,
  useState,
  type Dispatch,
  type ReactNode,
  type SetStateAction,
} from "react"

export type Locale = "zh-CN" | "en-US" | "ja-JP" | "es-ES"

const storageKey = "ctx.locale"

const defineCopy = <T extends Record<string, string>>(translations: T) =>
  translations

const englishCopy = defineCopy({
  appName: "CTX",
  language: "Language",
  languageChinese: "Chinese",
  languageEnglish: "English",
  languageJapanese: "Japanese",
  languageSpanish: "Spanish",
  navigationDocuments: "My documents",
  navigationSquare: "Square",
  navigationPersonalPage: "Personal page",
  navigationAdministration: "Administration",
  navigationAiRequests: "AI request audit",
  navigationSystemResources: "System resources",
  root: "Root",
  refreshDocuments: "Refresh documents",
  pageTitleDocuments: "My documents",
  pageTitleEditor: "Document editor",
  pageTitleAdministration: "Administration",
  pageTitleAiRequests: "AI request audit",
  pageTitleSystemResources: "System resources",
  pageTitleSquare: "Square",
  pageTitlePersonalPage: "Personal page",
  documentsTitle: "Your documents",
  documentsDescription:
    "Write in Markdown, keep every revision, and choose what to share.",
  newDocument: "New document",
  emptyDocumentsTitle: "Write the first one",
  emptyDocumentsDescription:
    "Start with a Markdown document. Save creates a revision; publishing makes it readable in the square.",
  createDocument: "Create document",
  openDocument: "Open document",
  backToDocuments: "Back to documents",
  updatedOn: "Updated {date}",
  publishedOn: "Published {date}",
  publicationTime: "Publication time",
  publicationTimeDescription:
    "Set the public date independently when this document was imported.",
  savePublicationTime: "Save publication time",
  publicationTimeSaved: "Publication time updated",
  document: "Document",
  title: "Title",
  titlePlaceholder: "Give this document a title",
  sourceLanguage: "Original language",
  sourceLanguagePlaceholder: "For example, zh-CN",
  sourceLanguageDescription:
    "Use a BCP 47 tag. Leave it as und and AI will infer the original language after publishing.",
  sourceLanguageAuto: "Detect after publishing",
  markdown: "Markdown",
  markdownDescription:
    "Markdown is the source. Each save writes an immutable revision.",
  editorWrite: "Write",
  editorPlaceholder: "Type / for blocks, or start writing…",
  editorCommands: "Blocks",
  editorInsertLink: "Insert link",
  editorInsertDocument: "Link document",
  editorInsertImage: "Insert image",
  editorUploadingImage: "Uploading image…",
  editorImageUploadFailed: "Image upload failed",
  editorInsertUrl: "Paste a URL",
  editorInsert: "Insert",
  editorSearchDocuments: "Search your documents",
  editorNoReferenceDocuments: "No other documents match",
  cancel: "Cancel",
  draft: "Draft",
  published: "Published",
  save: "Save",
  publish: "Publish",
  savedNewRevision: "Saved as a new revision",
  autosaving: "Saving changes…",
  autosavePending: "Changes waiting to save",
  autosaved: "All changes saved",
  autosaveFailed: "Save failed; this browser keeps a recovery copy",
  recoveredDocumentDraft: "Recovered unsaved changes from this browser",
  publishedCurrentRevision:
    "Published the original; AI work is continuing in the background",
  documentCreated: "Document created",
  revision: "Revision",
  viewPublicArticle: "View public article",
  editDocument: "Edit article",
  deleteDocument: "Delete document",
  deleteDocumentConfirm: "Delete permanently",
  documentDeleted: "Document deleted",
  documentNotFound: "Document not found",
  documentAi: "AI for this document",
  documentAiDescription:
    "AI works from the current revision and always leaves a reviewable result.",
  detectSourceLanguage: "Detect original language",
  polishDocument: "Polish document",
  appliedAiPolish: "Applied AI polish",
  applyPolish: "Apply polish",
  polishSaved: "Polished Markdown saved as a new revision",
  applyDetectedLanguage: "Use detected language",
  detectedSourceLanguage: "Detected original language: {language}",
  saveBeforeAiTitle: "Save before using AI",
  saveBeforeAi:
    "AI uses the latest saved revision. Save your changes before starting an AI task.",
  aiActions: "Actions",
  aiOutput: "Output",
  extractMetadata: "Extract metadata",
  aiWorkRecorded: "AI work recorded with this revision",
  appliedAiMetadata: "Applied AI metadata",
  applyMetadata: "Apply metadata",
  metadataSaved: "Metadata saved as a new revision",
  aiOutputEmpty:
    "Run a task to inspect an auditable AI result without changing your Markdown.",
  metadata: "Article metadata",
  metadataDescription: "Description",
  metadataSummary: "Summary",
  metadataTags: "Topics",
  metadataKeyPoints: "Key points",
  metadataAudience: "For",
  metadataPreparing: "Article metadata is being prepared in the background.",
  metadataWillBePrepared:
    "Article metadata will be extracted after publishing.",
  articleLanguage: "Article language",
  translationInProgressTitle: "Translation is being prepared",
  translationInProgress:
    "You are reading the original while the requested article and its editorial metadata are translated in the background.",
  squareTitle: "The square",
  squareDescription: "Published Markdown from everyone writing with CTX.",
  squareEmptyTitle: "Nothing has been published yet",
  squareEmptyDescription:
    "When someone publishes a document, it will appear here for everyone to read.",
  startWriting: "Start writing",
  readArticle: "Read article",
  backToSquare: "Back to the square",
  publishedDocumentNotFound: "Published document not found",
  rootConfigured: "Root user configured",
  initializeTitle: "Initialize CTX",
  initializeDescription:
    "The first authenticated Auth Mini user becomes CTX's root administrator. Sign-in stays owned by Auth Mini.",
  becomeRoot: "Become root administrator",
  aiConfigurationSaved: "AI configuration saved",
  administrationDescription:
    "Instance-only controls stay separate from writing and publishing.",
  openAiRouting: "OpenAI routing",
  openAiRoutingDescription:
    "CTX calls an OpenAI-compatible Responses API. The API key is encrypted on this host and never returned to the browser.",
  configured: "configured",
  needsKey: "needs key",
  baseUrl: "Base URL",
  model: "Model",
  apiKey: "API key",
  apiKeyPlaceholder: "Leave blank to keep the encrypted key",
  apiKeyDescription:
    "Only root can update this secret. CTX will never echo it back.",
  saveAiConfiguration: "Save AI configuration",
  aiRequestsTitle: "AI request audit",
  aiRequestsDescription:
    "Every metadata extraction, independent profile summary, and Markdown translation is recorded here.",
  aiRequestDocument: "Document",
  aiRequestTask: "Task",
  aiRequestTargetLanguage: "Target language",
  aiRequestStatus: "Status",
  aiRequestCreated: "Created",
  aiRequestCompleted: "Completed",
  aiRequestOpenAiLbRequestId: "OpenAI LB request ID",
  aiRequestResult: "Result",
  aiRequestError: "Error",
  aiRequestsEmpty: "No automatic AI requests have been recorded.",
  systemResourcesDescription:
    "A five-second view of the hosted CTX process, its storage, and the host it runs on.",
  systemResourcesRefreshes: "Refreshes every {seconds} seconds",
  systemResourcesSampledAt: "Sampled {date}",
  systemResourceCpu: "CPU",
  systemResourceLoad1m: "1m load",
  systemResourceLogicalCpus: "Logical CPUs",
  systemResourceMemory: "Memory",
  systemResourceProcessMemory: "CTX process",
  systemResourceOtherMemory: "Other system",
  systemResourceAvailableMemory: "Available",
  systemResourceSwap: "Swap",
  systemResourceNetwork: "Network",
  systemResourceReceived: "Received",
  systemResourceTransmitted: "Sent",
  systemResourceNetworkInterfaces: "Interfaces",
  systemResourceDisk: "Disk",
  systemResourceAvailableDisk: "Available",
  systemResourceMountPoint: "Mount",
  systemResourceSqlite: "SQLite database",
  systemResourceMainFile: "Main",
  systemResourceWalFile: "WAL",
  systemResourceShmFile: "Shared memory",
  systemResourceReclaimable: "Reclaimable",
  systemResourceUnavailable: "Unavailable",
  aiTaskMetadata: "Metadata",
  aiTaskTranslate: "Translation",
  aiTaskProfileExperience: "Profile experience",
  aiTaskProfilePersonality: "Profile personality",
  aiTaskProfileMbti: "Profile MBTI",
  aiTaskProfileSchwartz: "Profile Schwartz values",
  aiTaskProfileMotivations: "Profile motivations",
  aiTaskProfilePhilosophy: "Profile philosophy",
  aiTaskProfileTimeline: "Profile timeline",
  aiStatusQueued: "Queued",
  aiStatusRunning: "Running",
  aiStatusSucceeded: "Succeeded",
  aiStatusFailed: "Failed",
  personalPage: "Personal page",
  editPersonalPage: "Edit personal page",
  viewAuthorPage: "View profile",
  profileActivityTitle: "Publishing activity",
  profileActivityDescription: "Published articles over the last 52 weeks.",
  profilePublishedCount: "{count} published articles",
  profileActivityDay: "{date}: {count} published articles",
  profileActivityLess: "Less",
  profileActivityMore: "More",
  experienceSummary: "Experience resume",
  experienceSummaryDescription:
    "A factual resume generated from all published articles by this author.",
  experienceSummaryEmpty:
    "The author's published articles do not state experience facts yet.",
  profileTabResume: "Experience resume",
  profileTabPersonality: "Personality analysis",
  profileTabMbti: "MBTI",
  profileTabSchwartz: "Values",
  profileTabMotivations: "Motivations",
  profileTabPhilosophy: "Philosophy",
  profileTabTimeline: "Timeline",
  profileTabArticle: "Profile article",
  refreshProfileSummaries: "Refresh profile summaries",
  profileSummariesQueued: "Profile summary tasks queued",
  personalityAnalysis: "Personality & psychological reading",
  personalityAnalysisDescription:
    "An AI-generated, source-grounded reading of the author's published writing. It is not a clinical or diagnostic assessment.",
  personalityAnalysisEmpty:
    "The author's published articles do not yet contain enough explicit material for an evidence-supported reading.",
  mbtiTitle: "MBTI classification",
  mbtiDescription:
    "A tentative four-dimension reading of the author's published writing, with linked evidence for each dimension.",
  mbtiEmpty:
    "The published articles do not yet provide enough material for an MBTI classification.",
  mbtiClassification: "Classification",
  mbtiDimension: "Dimension",
  schwartzTitle: "Schwartz value tendencies",
  schwartzDescription:
    "A source-grounded ranking of the ten Schwartz value dimensions. Scores compare tendencies within this profile only.",
  schwartzEmpty:
    "The published articles do not yet provide enough material for a value-tendency ranking.",
  schwartzRadarTitle: "Schwartz value tendency radar chart",
  schwartzRadarCaption: "Higher scores indicate stronger textual evidence within this profile.",
  schwartzRank: "Rank",
  schwartzValue: "Value",
  schwartzScore: "Score",
  schwartzValueSelfDirection: "Self-direction",
  schwartzValueStimulation: "Stimulation",
  schwartzValueHedonism: "Hedonism",
  schwartzValueAchievement: "Achievement",
  schwartzValuePower: "Power",
  schwartzValueSecurity: "Security",
  schwartzValueConformity: "Conformity",
  schwartzValueTradition: "Tradition",
  schwartzValueBenevolence: "Benevolence",
  schwartzValueUniversalism: "Universalism",
  motivationsTitle: "Potential unconscious motivations",
  motivationsDescription:
    "A tentative, evidence-linked interpretation of recurring motivations expressed in the published writing.",
  motivationsEmpty:
    "The published articles do not yet provide enough material for a source-grounded motivation reading.",
  philosophyTitle: "Philosophical references",
  philosophyDescription:
    "Expressed propositions are read alongside relevant philosophical references, with their supporting articles.",
  philosophyEmpty:
    "The published articles do not yet provide enough material for a source-grounded philosophical reading.",
  timelineTitle: "Daily timeline",
  timelineDescription:
    "A date-by-date record of what the author did and the result stated in the published articles.",
  timelineEmpty:
    "No date-specific records can be supported by the published articles yet.",
  summaryResult: "Result",
  summaryConfidence: "Confidence",
  summaryConfidenceHigh: "High",
  summaryConfidenceMedium: "Medium",
  summaryConfidenceLow: "Low",
  summaryConfidenceUndetermined: "Undetermined",
  summaryEvidence: "Evidence",
  summaryNoEvidence: "No supporting article was identified.",
  profileArticle: "Profile article",
  profileArticleUnavailableTitle: "No published profile article",
  profileArticleUnavailableDescription:
    "This writer has not published a personal page yet.",
  profileNotFound: "Personal page not found",
  commentsTitle: "Comments",
  commentsDescription:
    "Discuss the whole article, or select a passage to leave an anchored comment.",
  commentsEmpty: "No comments yet. Start a thoughtful conversation.",
  writeComment: "Write a comment",
  commentPlaceholder: "Add to the discussion…",
  postComment: "Post comment",
  signInToComment: "Sign in to comment",
  commentOnSelection: "Comment on selection",
  selectedText: "Selected text",
  clearSelectionComment: "Comment on the whole article instead",
  inlineComment: "Passage comment",
  fullComment: "Article comment",
  commentPosted: "Comment posted",
  requestFailed: "CTX could not complete this request",
})

type TranslationKey = keyof typeof englishCopy
type Copy = Record<TranslationKey, string>

const chineseCopy: Copy = {
  appName: "CTX",
  language: "语言",
  languageChinese: "中文",
  languageEnglish: "英文",
  languageJapanese: "日文",
  languageSpanish: "西班牙文",
  navigationDocuments: "我的文档",
  navigationSquare: "广场",
  navigationPersonalPage: "个人主页",
  navigationAdministration: "管理",
  navigationAiRequests: "AI 请求审计",
  navigationSystemResources: "系统资源",
  root: "根管理员",
  refreshDocuments: "刷新文档",
  pageTitleDocuments: "我的文档",
  pageTitleEditor: "文档编辑器",
  pageTitleAdministration: "管理",
  pageTitleAiRequests: "AI 请求审计",
  pageTitleSystemResources: "系统资源",
  pageTitleSquare: "广场",
  pageTitlePersonalPage: "个人主页",
  documentsTitle: "你的文档",
  documentsDescription: "用 Markdown 写作，保留每次修订，并选择哪些内容公开。",
  newDocument: "新建文档",
  emptyDocumentsTitle: "写下第一篇",
  emptyDocumentsDescription:
    "从一篇 Markdown 文档开始。保存会创建修订版本；发布后，所有人都能在广场阅读。",
  createDocument: "创建文档",
  openDocument: "打开文档",
  backToDocuments: "返回文档",
  updatedOn: "更新于 {date}",
  publishedOn: "发布于 {date}",
  publicationTime: "发布时间",
  publicationTimeDescription: "导入的文档可以单独调整公开发布时间。",
  savePublicationTime: "保存发布时间",
  publicationTimeSaved: "发布时间已更新",
  document: "文档",
  title: "标题",
  titlePlaceholder: "给这篇文档取个标题",
  sourceLanguage: "原文语言",
  sourceLanguagePlaceholder: "例如 zh-CN",
  sourceLanguageDescription:
    "使用 BCP 47 语言标签。保留 und，发布后由 AI 推断原文语言。",
  sourceLanguageAuto: "发布后自动识别",
  markdown: "Markdown",
  markdownDescription:
    "Markdown 是事实来源。每次保存都会写入一个不可变修订版本。",
  editorWrite: "写作",
  editorPlaceholder: "输入 / 添加块，或直接开始写作…",
  editorCommands: "内容块",
  editorInsertLink: "插入链接",
  editorInsertDocument: "引用文档",
  editorInsertImage: "插入图片",
  editorUploadingImage: "正在上传图片…",
  editorImageUploadFailed: "图片上传失败",
  editorInsertUrl: "粘贴 URL",
  editorInsert: "插入",
  editorSearchDocuments: "搜索你的文档",
  editorNoReferenceDocuments: "没有匹配的其他文档",
  cancel: "取消",
  draft: "草稿",
  published: "已发布",
  save: "保存",
  publish: "发布",
  savedNewRevision: "已保存为新的修订版本",
  autosaving: "正在保存修改…",
  autosavePending: "修改等待保存",
  autosaved: "所有修改已保存",
  autosaveFailed: "保存失败；此浏览器已保留恢复副本",
  recoveredDocumentDraft: "已从此浏览器恢复未保存的修改",
  publishedCurrentRevision: "原文已发布；AI 任务正在后台继续执行",
  documentCreated: "文档已创建",
  revision: "修订版本",
  viewPublicArticle: "查看公开文章",
  editDocument: "编辑本文",
  deleteDocument: "删除文档",
  deleteDocumentConfirm: "永久删除",
  documentDeleted: "文档已删除",
  documentNotFound: "未找到文档",
  documentAi: "文档 AI",
  documentAiDescription: "AI 只基于当前修订版本工作，结果始终可供审查。",
  detectSourceLanguage: "识别原文语言",
  polishDocument: "润色文档",
  appliedAiPolish: "已应用 AI 润色",
  applyPolish: "应用润色",
  polishSaved: "润色后的 Markdown 已保存为新的修订版本",
  applyDetectedLanguage: "采用识别结果",
  detectedSourceLanguage: "识别出的原文语言：{language}",
  saveBeforeAiTitle: "先保存再使用 AI",
  saveBeforeAi:
    "AI 只会使用最近保存的修订版本。请先保存当前修改，再运行 AI 任务。",
  aiActions: "操作",
  aiOutput: "输出",
  extractMetadata: "提取元数据",
  aiWorkRecorded: "AI 工作已记录到此修订版本",
  appliedAiMetadata: "已应用 AI 元数据",
  applyMetadata: "应用元数据",
  metadataSaved: "元数据已保存为新的修订版本",
  aiOutputEmpty: "运行任务以查看可审查的 AI 结果，Markdown 不会被自动改写。",
  metadata: "文章元信息",
  metadataDescription: "简介",
  metadataSummary: "摘要",
  metadataTags: "主题",
  metadataKeyPoints: "要点",
  metadataAudience: "适合读者",
  metadataPreparing: "文章元信息正在后台准备。",
  metadataWillBePrepared: "发布后将自动提取文章元信息。",
  articleLanguage: "文章语言",
  translationInProgressTitle: "译文正在准备",
  translationInProgress: "当前显示原文；所选语言的文章和元信息正在后台生成。",
  squareTitle: "广场",
  squareDescription: "所有使用 CTX 写作者已经发布的 Markdown 文章。",
  squareEmptyTitle: "还没有文章发布",
  squareEmptyDescription: "任何人发布文档后，它都会出现在这里，供所有人阅读。",
  startWriting: "开始写作",
  readArticle: "阅读文章",
  backToSquare: "返回广场",
  publishedDocumentNotFound: "未找到已发布的文档",
  rootConfigured: "根管理员已配置",
  initializeTitle: "初始化 CTX",
  initializeDescription:
    "第一位完成认证的 Auth Mini 用户将成为 CTX 根管理员。登录仍由 Auth Mini 负责。",
  becomeRoot: "成为根管理员",
  aiConfigurationSaved: "AI 配置已保存",
  administrationDescription: "实例级控制与写作和发布保持分离。",
  openAiRouting: "OpenAI 路由",
  openAiRoutingDescription:
    "CTX 调用兼容 OpenAI 的 Responses API。API 密钥在此主机上加密，绝不会返回浏览器。",
  configured: "已配置",
  needsKey: "需要密钥",
  baseUrl: "基础 URL",
  model: "模型",
  apiKey: "API 密钥",
  apiKeyPlaceholder: "留空即可保留已加密的密钥",
  apiKeyDescription: "只有根管理员可以更新此密钥。CTX 不会将其返回。",
  saveAiConfiguration: "保存 AI 配置",
  aiRequestsTitle: "AI 请求审计",
  aiRequestsDescription:
    "元信息提取、独立主页总结和 Markdown 翻译都会记录在这里。",
  aiRequestDocument: "文档",
  aiRequestTask: "任务",
  aiRequestTargetLanguage: "目标语言",
  aiRequestStatus: "状态",
  aiRequestCreated: "创建时间",
  aiRequestCompleted: "完成时间",
  aiRequestOpenAiLbRequestId: "OpenAI LB 请求 ID",
  aiRequestResult: "结果",
  aiRequestError: "错误",
  aiRequestsEmpty: "尚未记录自动 AI 请求。",
  systemResourcesDescription:
    "每 5 秒采集一次 CTX 进程、存储和所在主机的资源快照。",
  systemResourcesRefreshes: "每 {seconds} 秒刷新",
  systemResourcesSampledAt: "采样时间 {date}",
  systemResourceCpu: "CPU",
  systemResourceLoad1m: "1 分钟负载",
  systemResourceLogicalCpus: "逻辑 CPU",
  systemResourceMemory: "内存",
  systemResourceProcessMemory: "CTX 进程",
  systemResourceOtherMemory: "其他系统",
  systemResourceAvailableMemory: "可用",
  systemResourceSwap: "交换空间",
  systemResourceNetwork: "网络",
  systemResourceReceived: "接收",
  systemResourceTransmitted: "发送",
  systemResourceNetworkInterfaces: "网卡",
  systemResourceDisk: "磁盘",
  systemResourceAvailableDisk: "可用",
  systemResourceMountPoint: "挂载点",
  systemResourceSqlite: "SQLite 数据库",
  systemResourceMainFile: "主文件",
  systemResourceWalFile: "WAL",
  systemResourceShmFile: "共享内存",
  systemResourceReclaimable: "可回收",
  systemResourceUnavailable: "不可用",
  aiTaskMetadata: "元信息提取",
  aiTaskTranslate: "翻译",
  aiTaskProfileExperience: "主页经历总结",
  aiTaskProfilePersonality: "主页人格分析",
  aiTaskProfileMbti: "主页 MBTI",
  aiTaskProfileSchwartz: "主页施瓦茨价值观",
  aiTaskProfileMotivations: "主页动机分析",
  aiTaskProfilePhilosophy: "主页哲学参照",
  aiTaskProfileTimeline: "主页日记时间线",
  aiStatusQueued: "排队中",
  aiStatusRunning: "执行中",
  aiStatusSucceeded: "成功",
  aiStatusFailed: "失败",
  personalPage: "个人主页",
  editPersonalPage: "编辑个人主页",
  viewAuthorPage: "查看主页",
  profileActivityTitle: "发布活动",
  profileActivityDescription: "过去 52 周已发布的文章。",
  profilePublishedCount: "已发布 {count} 篇文章",
  profileActivityDay: "{date}：发布 {count} 篇文章",
  profileActivityLess: "少",
  profileActivityMore: "多",
  experienceSummary: "经历简历",
  experienceSummaryDescription:
    "根据该作者全部已发布文章自动生成的客观中立简历。",
  experienceSummaryEmpty: "该作者已发布的文章暂未包含可整理为简历的经历事实。",
  profileTabResume: "经历简历",
  profileTabPersonality: "人格心理分析",
  profileTabMbti: "MBTI",
  profileTabSchwartz: "价值观",
  profileTabMotivations: "潜意识动机",
  profileTabPhilosophy: "哲学参照",
  profileTabTimeline: "每日时间线",
  profileTabArticle: "主页文章",
  refreshProfileSummaries: "重新生成主页总结",
  profileSummariesQueued: "主页总结任务已排队",
  personalityAnalysis: "人格与心理文本分析",
  personalityAnalysisDescription:
    "基于作者已发布文章的 AI 文本分析，只呈现可追溯的写作观察；这不是医疗、精神健康或临床诊断。",
  personalityAnalysisEmpty:
    "该作者已发布的文章暂未包含足以形成有证据支撑分析的明确内容。",
  mbtiTitle: "MBTI 分类",
  mbtiDescription: "对作者公开文章的四维度暂定解读；每个维度均附可跳转的证据。",
  mbtiEmpty: "公开文章暂未提供足以形成 MBTI 分类的材料。",
  mbtiClassification: "分类结果",
  mbtiDimension: "维度",
  schwartzTitle: "施瓦茨价值观倾向",
  schwartzDescription:
    "基于文章证据对十项施瓦茨价值观进行排序。分数仅用于比较该作者自身的文本倾向。",
  schwartzEmpty: "公开文章暂未提供足以形成价值观倾向排序的材料。",
  schwartzRadarTitle: "施瓦茨价值观倾向雷达图",
  schwartzRadarCaption: "分数越高，表示该作者文章中与该价值观相关的证据越强。",
  schwartzRank: "排序",
  schwartzValue: "价值观",
  schwartzScore: "分数",
  schwartzValueSelfDirection: "自我导向",
  schwartzValueStimulation: "刺激",
  schwartzValueHedonism: "享乐主义",
  schwartzValueAchievement: "成就",
  schwartzValuePower: "权力",
  schwartzValueSecurity: "安全",
  schwartzValueConformity: "遵从",
  schwartzValueTradition: "传统",
  schwartzValueBenevolence: "仁爱",
  schwartzValueUniversalism: "普遍主义",
  motivationsTitle: "潜意识动机分析",
  motivationsDescription: "基于反复出现的公开表达，对潜在动机所作的暂定、可追溯解读。",
  motivationsEmpty: "公开文章暂未提供足以形成有来源支撑的动机解读的材料。",
  philosophyTitle: "哲学参照",
  philosophyDescription: "将作者明确表达的命题与相关哲学参照并置，并给出支持它们的文章。",
  philosophyEmpty: "公开文章暂未提供足以形成有来源支撑的哲学解读的材料。",
  timelineTitle: "每日时间线",
  timelineDescription: "按日期记录作者做了什么，以及公开文章明确记载的结果。",
  timelineEmpty: "公开文章暂未包含可由日期支撑的记录。",
  summaryResult: "结论",
  summaryConfidence: "置信度",
  summaryConfidenceHigh: "高",
  summaryConfidenceMedium: "中",
  summaryConfidenceLow: "低",
  summaryConfidenceUndetermined: "未确定",
  summaryEvidence: "证据",
  summaryNoEvidence: "未识别到支持该项的文章。",
  profileArticle: "主页文章",
  profileArticleUnavailableTitle: "尚未发布主页文章",
  profileArticleUnavailableDescription: "这位作者尚未发布个人主页。",
  profileNotFound: "未找到个人主页",
  commentsTitle: "评论",
  commentsDescription: "可以讨论全文，也可以划线选中段落后留下锚定评论。",
  commentsEmpty: "还没有评论。开始一段有价值的讨论吧。",
  writeComment: "撰写评论",
  commentPlaceholder: "参与讨论…",
  postComment: "发布评论",
  signInToComment: "登录后评论",
  commentOnSelection: "评论选中内容",
  selectedText: "选中的文字",
  clearSelectionComment: "改为评论全文",
  inlineComment: "划线评论",
  fullComment: "全文评论",
  commentPosted: "评论已发布",
  requestFailed: "CTX 无法完成此请求",
}

const japaneseCopy: Copy = {
  ...englishCopy,
  language: "言語",
  languageChinese: "中国語",
  languageEnglish: "英語",
  languageJapanese: "日本語",
  languageSpanish: "スペイン語",
  navigationDocuments: "自分の文書",
  navigationSquare: "広場",
  navigationPersonalPage: "個人ページ",
  navigationAdministration: "管理",
  navigationAiRequests: "AI リクエスト監査",
  navigationSystemResources: "システムリソース",
  root: "ルート管理者",
  refreshDocuments: "文書を更新",
  pageTitleDocuments: "自分の文書",
  pageTitleEditor: "文書エディター",
  pageTitleAdministration: "管理",
  pageTitleAiRequests: "AI リクエスト監査",
  pageTitleSystemResources: "システムリソース",
  pageTitleSquare: "広場",
  pageTitlePersonalPage: "個人ページ",
  documentsTitle: "あなたの文書",
  documentsDescription:
    "Markdown で書き、すべてのリビジョンを残し、共有する内容を選べます。",
  newDocument: "新しい文書",
  emptyDocumentsTitle: "最初の一篇を書く",
  emptyDocumentsDescription:
    "Markdown 文書から始めましょう。保存するとリビジョンが作成され、公開すると広場で読めるようになります。",
  createDocument: "文書を作成",
  openDocument: "文書を開く",
  backToDocuments: "文書に戻る",
  updatedOn: "更新日 {date}",
  publishedOn: "公開日 {date}",
  publicationTime: "公開日時",
  publicationTimeDescription:
    "インポートした文書の公開日時を個別に設定します。",
  savePublicationTime: "公開日時を保存",
  publicationTimeSaved: "公開日時を更新しました",
  document: "文書",
  title: "タイトル",
  titlePlaceholder: "この文書のタイトルを入力",
  sourceLanguage: "原文の言語",
  sourceLanguagePlaceholder: "例: ja-JP",
  sourceLanguageDescription:
    "BCP 47 言語タグを使います。und のまま公開すると、AI が原文の言語を推定します。",
  sourceLanguageAuto: "公開後に自動検出",
  markdownDescription:
    "Markdown が原典です。保存のたびに不変のリビジョンを作成します。",
  editorWrite: "編集",
  editorPlaceholder: "/ を入力してブロックを追加、または書き始めます…",
  editorCommands: "ブロック",
  editorInsertLink: "リンクを挿入",
  editorInsertDocument: "文書を参照",
  editorInsertImage: "画像を挿入",
  editorUploadingImage: "画像をアップロードしています…",
  editorImageUploadFailed: "画像のアップロードに失敗しました",
  editorInsertUrl: "URL を貼り付け",
  editorInsert: "挿入",
  editorSearchDocuments: "自分の文書を検索",
  editorNoReferenceDocuments: "一致する他の文書はありません",
  cancel: "キャンセル",
  draft: "下書き",
  published: "公開済み",
  save: "保存",
  publish: "公開",
  savedNewRevision: "新しいリビジョンとして保存しました",
  autosaving: "変更を保存しています…",
  autosavePending: "変更は保存待ちです",
  autosaved: "すべての変更を保存しました",
  autosaveFailed: "保存に失敗しました。このブラウザーに復元用コピーがあります",
  recoveredDocumentDraft: "このブラウザーから未保存の変更を復元しました",
  publishedCurrentRevision:
    "原文を公開しました。AI 処理はバックグラウンドで継続しています",
  documentCreated: "文書を作成しました",
  revision: "リビジョン",
  viewPublicArticle: "公開記事を表示",
  editDocument: "記事を編集",
  deleteDocument: "文書を削除",
  deleteDocumentConfirm: "完全に削除",
  documentDeleted: "文書を削除しました",
  documentNotFound: "文書が見つかりません",
  documentAi: "この文書の AI",
  documentAiDescription:
    "AI は現在のリビジョンだけを使用し、確認可能な結果を残します。",
  detectSourceLanguage: "原文の言語を検出",
  polishDocument: "文書を推敲",
  appliedAiPolish: "AI による推敲を適用",
  applyPolish: "推敲を適用",
  polishSaved: "推敲した Markdown を新しいリビジョンとして保存しました",
  applyDetectedLanguage: "検出した言語を使用",
  detectedSourceLanguage: "検出した原文の言語: {language}",
  saveBeforeAiTitle: "AI を使う前に保存",
  saveBeforeAi:
    "AI は最新の保存済みリビジョンを使用します。先に変更を保存してください。",
  aiActions: "操作",
  aiOutput: "出力",
  extractMetadata: "メタデータを抽出",
  aiWorkRecorded: "AI 処理をこのリビジョンに記録しました",
  appliedAiMetadata: "AI メタデータを適用しました",
  applyMetadata: "メタデータを適用",
  metadataSaved: "メタデータを新しいリビジョンとして保存しました",
  aiOutputEmpty:
    "Markdown を変更せずに、監査可能な AI 結果を確認するにはタスクを実行してください。",
  metadata: "記事メタデータ",
  metadataDescription: "説明",
  metadataSummary: "要約",
  metadataTags: "トピック",
  metadataKeyPoints: "要点",
  metadataAudience: "対象読者",
  metadataPreparing: "記事メタデータをバックグラウンドで準備しています。",
  metadataWillBePrepared: "公開後に記事メタデータを抽出します。",
  articleLanguage: "記事の言語",
  translationInProgressTitle: "翻訳を準備しています",
  translationInProgress:
    "要求された言語の記事とメタデータをバックグラウンドで翻訳しているため、原文を表示しています。",
  squareTitle: "広場",
  squareDescription: "CTX で書いたすべての人の公開 Markdown です。",
  squareEmptyTitle: "まだ公開された記事はありません",
  squareEmptyDescription:
    "誰かが文書を公開すると、誰でもここで読めるようになります。",
  startWriting: "書き始める",
  readArticle: "記事を読む",
  backToSquare: "広場に戻る",
  publishedDocumentNotFound: "公開文書が見つかりません",
  rootConfigured: "ルート管理者を設定しました",
  initializeTitle: "CTX を初期化",
  initializeDescription:
    "最初に認証された Auth Mini ユーザーが CTX のルート管理者になります。サインインは Auth Mini が管理します。",
  becomeRoot: "ルート管理者になる",
  aiConfigurationSaved: "AI 設定を保存しました",
  administrationDescription:
    "インスタンス固有の操作は、執筆と公開から分離されています。",
  openAiRouting: "OpenAI ルーティング",
  openAiRoutingDescription:
    "CTX は OpenAI 互換の Responses API を呼び出します。API キーはこのホストで暗号化され、ブラウザーには返されません。",
  configured: "設定済み",
  needsKey: "キーが必要",
  baseUrl: "ベース URL",
  model: "モデル",
  apiKey: "API キー",
  apiKeyPlaceholder: "暗号化済みキーを保持する場合は空欄のままにします",
  apiKeyDescription:
    "この秘密情報を更新できるのはルート管理者だけです。CTX は再表示しません。",
  saveAiConfiguration: "AI 設定を保存",
  aiRequestsTitle: "AI リクエスト監査",
  aiRequestsDescription:
    "メタデータ抽出、独立したプロフィール要約、Markdown 翻訳をすべて記録します。",
  aiRequestDocument: "文書",
  aiRequestTask: "タスク",
  aiRequestTargetLanguage: "対象言語",
  aiRequestStatus: "状態",
  aiRequestCreated: "作成日時",
  aiRequestCompleted: "完了日時",
  aiRequestOpenAiLbRequestId: "OpenAI LB リクエスト ID",
  aiRequestResult: "結果",
  aiRequestError: "エラー",
  aiRequestsEmpty: "自動 AI リクエストはまだありません。",
  systemResourcesDescription:
    "ホスト上の CTX プロセスとストレージのリソースを 5 秒ごとに確認します。",
  systemResourcesRefreshes: "{seconds} 秒ごとに更新",
  systemResourcesSampledAt: "サンプリング時刻 {date}",
  systemResourceCpu: "CPU",
  systemResourceLoad1m: "1 分間の負荷",
  systemResourceLogicalCpus: "論理 CPU",
  systemResourceMemory: "メモリ",
  systemResourceProcessMemory: "CTX プロセス",
  systemResourceOtherMemory: "その他のシステム",
  systemResourceAvailableMemory: "利用可能",
  systemResourceSwap: "スワップ",
  systemResourceNetwork: "ネットワーク",
  systemResourceReceived: "受信",
  systemResourceTransmitted: "送信",
  systemResourceNetworkInterfaces: "インターフェース",
  systemResourceDisk: "ディスク",
  systemResourceAvailableDisk: "利用可能",
  systemResourceMountPoint: "マウントポイント",
  systemResourceSqlite: "SQLite データベース",
  systemResourceMainFile: "メイン",
  systemResourceWalFile: "WAL",
  systemResourceShmFile: "共有メモリ",
  systemResourceReclaimable: "回収可能",
  systemResourceUnavailable: "利用不可",
  aiTaskMetadata: "メタデータ抽出",
  aiTaskTranslate: "翻訳",
  aiTaskProfileExperience: "プロフィール経歴要約",
  aiTaskProfilePersonality: "プロフィール人格分析",
  aiTaskProfileMbti: "プロフィール MBTI",
  aiTaskProfileSchwartz: "プロフィール・シュワルツ価値観",
  aiTaskProfileMotivations: "プロフィール動機分析",
  aiTaskProfilePhilosophy: "プロフィール哲学参照",
  aiTaskProfileTimeline: "プロフィール日別タイムライン",
  aiStatusQueued: "待機中",
  aiStatusRunning: "実行中",
  aiStatusSucceeded: "成功",
  aiStatusFailed: "失敗",
  personalPage: "個人ページ",
  editPersonalPage: "個人ページを編集",
  viewAuthorPage: "プロフィールを見る",
  profileActivityTitle: "公開アクティビティ",
  profileActivityDescription: "過去 52 週間に公開した記事です。",
  profilePublishedCount: "公開済みの記事 {count} 件",
  profileActivityDay: "{date}: 公開済みの記事 {count} 件",
  profileActivityLess: "少",
  profileActivityMore: "多",
  experienceSummary: "経歴レジュメ",
  experienceSummaryDescription:
    "この執筆者が公開したすべての記事から自動生成された、客観的で中立なレジュメです。",
  experienceSummaryEmpty:
    "この執筆者が公開した記事には、レジュメに整理できる経歴の事実がまだありません。",
  profileTabResume: "経歴レジュメ",
  profileTabPersonality: "パーソナリティ分析",
  profileTabMbti: "MBTI",
  profileTabSchwartz: "価値観",
  profileTabMotivations: "潜在的動機",
  profileTabPhilosophy: "哲学的参照",
  profileTabTimeline: "日別タイムライン",
  profileTabArticle: "プロフィール記事",
  refreshProfileSummaries: "プロフィール要約を更新",
  profileSummariesQueued: "プロフィール要約タスクをキューに追加しました",
  personalityAnalysis: "パーソナリティと心理の文章分析",
  personalityAnalysisDescription:
    "公開された文章に基づく AI の分析であり、根拠をたどれる記述だけを扱います。臨床的または診断的な評価ではありません。",
  personalityAnalysisEmpty:
    "この執筆者が公開した記事には、根拠のある分析に十分な明示的な内容がまだありません。",
  mbtiTitle: "MBTI 分類",
  mbtiDescription: "公開された文章を四つの次元から暫定的に読み解き、各次元に根拠の記事を付けます。",
  mbtiEmpty: "公開された記事には、MBTI 分類に十分な材料がまだありません。",
  mbtiClassification: "分類",
  mbtiDimension: "次元",
  schwartzTitle: "シュワルツ価値観の傾向",
  schwartzDescription:
    "記事の根拠に基づく、シュワルツの十価値観の順位です。スコアはこのプロフィール内でのみ比較できます。",
  schwartzEmpty: "公開された記事には、価値観の傾向を順位付けする十分な材料がまだありません。",
  schwartzRadarTitle: "シュワルツ価値観傾向のレーダーチャート",
  schwartzRadarCaption: "スコアが高いほど、このプロフィールの文章で根拠が強いことを示します。",
  schwartzRank: "順位",
  schwartzValue: "価値観",
  schwartzScore: "スコア",
  schwartzValueSelfDirection: "自己方向づけ",
  schwartzValueStimulation: "刺激",
  schwartzValueHedonism: "快楽主義",
  schwartzValueAchievement: "達成",
  schwartzValuePower: "権力",
  schwartzValueSecurity: "安全",
  schwartzValueConformity: "同調",
  schwartzValueTradition: "伝統",
  schwartzValueBenevolence: "慈善",
  schwartzValueUniversalism: "普遍主義",
  motivationsTitle: "潜在的動機の分析",
  motivationsDescription: "公開文章で繰り返し表現された動機を、根拠とともに暫定的に解釈します。",
  motivationsEmpty: "公開された記事には、根拠のある動機の解釈に十分な材料がまだありません。",
  philosophyTitle: "哲学的参照",
  philosophyDescription: "作者が明示した命題を関連する哲学的参照と並べ、根拠となる記事を示します。",
  philosophyEmpty: "公開された記事には、根拠のある哲学的解釈に十分な材料がまだありません。",
  timelineTitle: "日別タイムライン",
  timelineDescription: "作者が行ったことと記事中に記録された結果を、日付ごとに整理します。",
  timelineEmpty: "公開された記事には、日付で裏付けられる記録がまだありません。",
  summaryResult: "結論",
  summaryConfidence: "確信度",
  summaryConfidenceHigh: "高",
  summaryConfidenceMedium: "中",
  summaryConfidenceLow: "低",
  summaryConfidenceUndetermined: "未確定",
  summaryEvidence: "根拠",
  summaryNoEvidence: "裏付けとなる記事は特定されていません。",
  profileArticle: "プロフィール記事",
  profileArticleUnavailableTitle: "公開されたプロフィール記事はありません",
  profileArticleUnavailableDescription:
    "この執筆者はまだ個人ページを公開していません。",
  profileNotFound: "個人ページが見つかりません",
  commentsTitle: "コメント",
  commentsDescription:
    "記事全体について話すか、文章を選択してアンカー付きのコメントを残せます。",
  commentsEmpty: "まだコメントはありません。会話を始めましょう。",
  writeComment: "コメントを書く",
  commentPlaceholder: "議論に加わる…",
  postComment: "コメントを投稿",
  signInToComment: "コメントするにはサインイン",
  commentOnSelection: "選択箇所にコメント",
  selectedText: "選択したテキスト",
  clearSelectionComment: "記事全体にコメントする",
  inlineComment: "文章へのコメント",
  fullComment: "記事へのコメント",
  commentPosted: "コメントを投稿しました",
  requestFailed: "CTX はこのリクエストを完了できませんでした",
}

const spanishCopy: Copy = {
  ...englishCopy,
  language: "Idioma",
  languageChinese: "Chino",
  languageEnglish: "Inglés",
  languageJapanese: "Japonés",
  languageSpanish: "Español",
  navigationDocuments: "Mis documentos",
  navigationSquare: "Plaza",
  navigationPersonalPage: "Página personal",
  navigationAdministration: "Administración",
  navigationAiRequests: "Auditoría de solicitudes de IA",
  navigationSystemResources: "Recursos del sistema",
  root: "Administrador raíz",
  refreshDocuments: "Actualizar documentos",
  pageTitleDocuments: "Mis documentos",
  pageTitleEditor: "Editor de documentos",
  pageTitleAdministration: "Administración",
  pageTitleAiRequests: "Auditoría de solicitudes de IA",
  pageTitleSystemResources: "Recursos del sistema",
  pageTitleSquare: "Plaza",
  pageTitlePersonalPage: "Página personal",
  documentsTitle: "Tus documentos",
  documentsDescription:
    "Escribe en Markdown, conserva cada revisión y decide qué compartir.",
  newDocument: "Nuevo documento",
  emptyDocumentsTitle: "Escribe el primero",
  emptyDocumentsDescription:
    "Empieza con un documento Markdown. Guardar crea una revisión; publicar lo hace legible en la plaza.",
  createDocument: "Crear documento",
  openDocument: "Abrir documento",
  backToDocuments: "Volver a documentos",
  updatedOn: "Actualizado {date}",
  publishedOn: "Publicado {date}",
  publicationTime: "Fecha de publicación",
  publicationTimeDescription:
    "Ajusta la fecha pública por separado cuando el documento fue importado.",
  savePublicationTime: "Guardar fecha de publicación",
  publicationTimeSaved: "Fecha de publicación actualizada",
  document: "Documento",
  title: "Título",
  titlePlaceholder: "Pon un título a este documento",
  sourceLanguage: "Idioma original",
  sourceLanguagePlaceholder: "Por ejemplo, es-ES",
  sourceLanguageDescription:
    "Usa una etiqueta BCP 47. Si dejas und, la IA inferirá el idioma original después de publicar.",
  sourceLanguageAuto: "Detectar después de publicar",
  markdownDescription:
    "Markdown es la fuente. Cada guardado escribe una revisión inmutable.",
  editorWrite: "Escribir",
  editorPlaceholder: "Escribe / para añadir bloques o empieza a escribir…",
  editorCommands: "Bloques",
  editorInsertLink: "Insertar enlace",
  editorInsertDocument: "Vincular documento",
  editorInsertImage: "Insertar imagen",
  editorUploadingImage: "Subiendo imagen…",
  editorImageUploadFailed: "No se pudo subir la imagen",
  editorInsertUrl: "Pega una URL",
  editorInsert: "Insertar",
  editorSearchDocuments: "Busca en tus documentos",
  editorNoReferenceDocuments: "No hay otros documentos coincidentes",
  cancel: "Cancelar",
  draft: "Borrador",
  published: "Publicado",
  save: "Guardar",
  publish: "Publicar",
  savedNewRevision: "Guardado como una nueva revisión",
  autosaving: "Guardando cambios…",
  autosavePending: "Cambios pendientes de guardar",
  autosaved: "Todos los cambios guardados",
  autosaveFailed:
    "No se pudo guardar; este navegador conserva una copia de recuperación",
  recoveredDocumentDraft:
    "Se recuperaron cambios sin guardar de este navegador",
  publishedCurrentRevision:
    "El original se publicó; el trabajo de IA continúa en segundo plano",
  documentCreated: "Documento creado",
  revision: "Revisión",
  viewPublicArticle: "Ver artículo público",
  editDocument: "Editar artículo",
  deleteDocument: "Eliminar documento",
  deleteDocumentConfirm: "Eliminar definitivamente",
  documentDeleted: "Documento eliminado",
  documentNotFound: "Documento no encontrado",
  documentAi: "IA para este documento",
  documentAiDescription:
    "La IA trabaja con la revisión actual y siempre deja un resultado revisable.",
  detectSourceLanguage: "Detectar idioma original",
  polishDocument: "Mejorar el documento",
  appliedAiPolish: "Mejora de IA aplicada",
  applyPolish: "Aplicar mejora",
  polishSaved: "El Markdown mejorado se guardó como una nueva revisión",
  applyDetectedLanguage: "Usar idioma detectado",
  detectedSourceLanguage: "Idioma original detectado: {language}",
  saveBeforeAiTitle: "Guarda antes de usar IA",
  saveBeforeAi:
    "La IA usa la última revisión guardada. Guarda tus cambios antes de iniciar una tarea de IA.",
  aiActions: "Acciones",
  aiOutput: "Resultado",
  extractMetadata: "Extraer metadatos",
  aiWorkRecorded: "El trabajo de IA se registró con esta revisión",
  appliedAiMetadata: "Metadatos de IA aplicados",
  applyMetadata: "Aplicar metadatos",
  metadataSaved: "Metadatos guardados como una nueva revisión",
  aiOutputEmpty:
    "Ejecuta una tarea para revisar un resultado de IA auditable sin cambiar tu Markdown.",
  metadata: "Metadatos del artículo",
  metadataDescription: "Descripción",
  metadataSummary: "Resumen",
  metadataTags: "Temas",
  metadataKeyPoints: "Puntos clave",
  metadataAudience: "Dirigido a",
  metadataPreparing:
    "Los metadatos del artículo se están preparando en segundo plano.",
  metadataWillBePrepared:
    "Los metadatos del artículo se extraerán después de publicar.",
  articleLanguage: "Idioma del artículo",
  translationInProgressTitle: "La traducción se está preparando",
  translationInProgress:
    "Estás leyendo el original mientras el artículo y sus metadatos se traducen en segundo plano.",
  squareTitle: "La plaza",
  squareDescription:
    "Markdown publicado por todas las personas que escriben con CTX.",
  squareEmptyTitle: "Aún no se ha publicado nada",
  squareEmptyDescription:
    "Cuando alguien publique un documento, aparecerá aquí para que todos lo lean.",
  startWriting: "Empezar a escribir",
  readArticle: "Leer artículo",
  backToSquare: "Volver a la plaza",
  publishedDocumentNotFound: "Documento publicado no encontrado",
  rootConfigured: "Usuario raíz configurado",
  initializeTitle: "Inicializar CTX",
  initializeDescription:
    "El primer usuario autenticado de Auth Mini se convierte en administrador raíz de CTX. Auth Mini sigue siendo responsable del inicio de sesión.",
  becomeRoot: "Convertirme en administrador raíz",
  aiConfigurationSaved: "Configuración de IA guardada",
  administrationDescription:
    "Los controles exclusivos de la instancia permanecen separados de la escritura y la publicación.",
  openAiRouting: "Enrutamiento de OpenAI",
  openAiRoutingDescription:
    "CTX llama a una API Responses compatible con OpenAI. La clave de API se cifra en este host y nunca se devuelve al navegador.",
  configured: "configurado",
  needsKey: "requiere clave",
  baseUrl: "URL base",
  model: "Modelo",
  apiKey: "Clave de API",
  apiKeyPlaceholder: "Déjalo vacío para conservar la clave cifrada",
  apiKeyDescription:
    "Solo la raíz puede actualizar este secreto. CTX nunca lo mostrará de nuevo.",
  saveAiConfiguration: "Guardar configuración de IA",
  aiRequestsTitle: "Auditoría de solicitudes de IA",
  aiRequestsDescription:
    "Cada extracción de metadatos, resumen de perfil independiente y traducción Markdown se registra aquí.",
  aiRequestDocument: "Documento",
  aiRequestTask: "Tarea",
  aiRequestTargetLanguage: "Idioma de destino",
  aiRequestStatus: "Estado",
  aiRequestCreated: "Creado",
  aiRequestCompleted: "Completado",
  aiRequestOpenAiLbRequestId: "ID de solicitud de OpenAI LB",
  aiRequestResult: "Resultado",
  aiRequestError: "Error",
  aiRequestsEmpty: "No se han registrado solicitudes automáticas de IA.",
  systemResourcesDescription:
    "Una vista cada cinco segundos del proceso CTX alojado, su almacenamiento y el host.",
  systemResourcesRefreshes: "Se actualiza cada {seconds} segundos",
  systemResourcesSampledAt: "Muestra {date}",
  systemResourceCpu: "CPU",
  systemResourceLoad1m: "Carga de 1 min",
  systemResourceLogicalCpus: "CPU lógicas",
  systemResourceMemory: "Memoria",
  systemResourceProcessMemory: "Proceso CTX",
  systemResourceOtherMemory: "Otro sistema",
  systemResourceAvailableMemory: "Disponible",
  systemResourceSwap: "Intercambio",
  systemResourceNetwork: "Red",
  systemResourceReceived: "Recibido",
  systemResourceTransmitted: "Enviado",
  systemResourceNetworkInterfaces: "Interfaces",
  systemResourceDisk: "Disco",
  systemResourceAvailableDisk: "Disponible",
  systemResourceMountPoint: "Montaje",
  systemResourceSqlite: "Base de datos SQLite",
  systemResourceMainFile: "Principal",
  systemResourceWalFile: "WAL",
  systemResourceShmFile: "Memoria compartida",
  systemResourceReclaimable: "Recuperable",
  systemResourceUnavailable: "No disponible",
  aiTaskMetadata: "Metadatos",
  aiTaskTranslate: "Traducción",
  aiTaskProfileExperience: "Resumen de experiencia del perfil",
  aiTaskProfilePersonality: "Personalidad del perfil",
  aiTaskProfileMbti: "MBTI del perfil",
  aiTaskProfileSchwartz: "Valores de Schwartz del perfil",
  aiTaskProfileMotivations: "Motivaciones del perfil",
  aiTaskProfilePhilosophy: "Filosofía del perfil",
  aiTaskProfileTimeline: "Cronología del perfil",
  aiStatusQueued: "En cola",
  aiStatusRunning: "En curso",
  aiStatusSucceeded: "Completado",
  aiStatusFailed: "Fallido",
  personalPage: "Página personal",
  editPersonalPage: "Editar página personal",
  viewAuthorPage: "Ver perfil",
  profileActivityTitle: "Actividad de publicación",
  profileActivityDescription:
    "Artículos publicados durante las últimas 52 semanas.",
  profilePublishedCount: "{count} artículos publicados",
  profileActivityDay: "{date}: {count} artículos publicados",
  profileActivityLess: "Menos",
  profileActivityMore: "Más",
  experienceSummary: "Currículum de experiencia",
  experienceSummaryDescription:
    "Currículum objetivo generado a partir de todos los artículos publicados por esta persona.",
  experienceSummaryEmpty:
    "Los artículos publicados por esta persona aún no contienen hechos de experiencia.",
  profileTabResume: "Currículum",
  profileTabPersonality: "Análisis de personalidad",
  profileTabMbti: "MBTI",
  profileTabSchwartz: "Valores",
  profileTabMotivations: "Motivaciones",
  profileTabPhilosophy: "Filosofía",
  profileTabTimeline: "Cronología",
  profileTabArticle: "Artículo de perfil",
  refreshProfileSummaries: "Actualizar resúmenes del perfil",
  profileSummariesQueued: "Tareas de resumen del perfil en cola",
  personalityAnalysis: "Lectura de personalidad y psicología",
  personalityAnalysisDescription:
    "Una lectura de IA basada en evidencia de los textos publicados por esta persona. No es una evaluación clínica ni diagnóstica.",
  personalityAnalysisEmpty:
    "Los artículos publicados por esta persona aún no contienen suficiente material explícito para una lectura respaldada por evidencia.",
  mbtiTitle: "Clasificación MBTI",
  mbtiDescription:
    "Una lectura provisional en cuatro dimensiones de los textos publicados, con evidencia enlazada para cada dimensión.",
  mbtiEmpty:
    "Los artículos publicados aún no ofrecen suficiente material para una clasificación MBTI.",
  mbtiClassification: "Clasificación",
  mbtiDimension: "Dimensión",
  schwartzTitle: "Tendencias de valores de Schwartz",
  schwartzDescription:
    "Una clasificación basada en fuentes de las diez dimensiones de valores de Schwartz. Las puntuaciones solo comparan tendencias dentro de este perfil.",
  schwartzEmpty:
    "Los artículos publicados aún no ofrecen suficiente material para una clasificación de tendencias de valores.",
  schwartzRadarTitle: "Gráfico radar de tendencias de valores de Schwartz",
  schwartzRadarCaption:
    "Una puntuación más alta indica evidencia textual más fuerte dentro de este perfil.",
  schwartzRank: "Rango",
  schwartzValue: "Valor",
  schwartzScore: "Puntuación",
  schwartzValueSelfDirection: "Autodirección",
  schwartzValueStimulation: "Estimulación",
  schwartzValueHedonism: "Hedonismo",
  schwartzValueAchievement: "Logro",
  schwartzValuePower: "Poder",
  schwartzValueSecurity: "Seguridad",
  schwartzValueConformity: "Conformidad",
  schwartzValueTradition: "Tradición",
  schwartzValueBenevolence: "Benevolencia",
  schwartzValueUniversalism: "Universalismo",
  motivationsTitle: "Posibles motivaciones inconscientes",
  motivationsDescription:
    "Una interpretación provisional y vinculada a evidencia de motivaciones recurrentes expresadas en los textos publicados.",
  motivationsEmpty:
    "Los artículos publicados aún no ofrecen suficiente material para una lectura de motivaciones basada en fuentes.",
  philosophyTitle: "Referencias filosóficas",
  philosophyDescription:
    "Las proposiciones expresadas se leen junto a referencias filosóficas pertinentes y sus artículos de respaldo.",
  philosophyEmpty:
    "Los artículos publicados aún no ofrecen suficiente material para una lectura filosófica basada en fuentes.",
  timelineTitle: "Cronología diaria",
  timelineDescription:
    "Un registro por fecha de lo que hizo la persona autora y del resultado declarado en los artículos publicados.",
  timelineEmpty:
    "Los artículos publicados aún no contienen registros específicos por fecha que puedan respaldarse.",
  summaryResult: "Resultado",
  summaryConfidence: "Confianza",
  summaryConfidenceHigh: "Alta",
  summaryConfidenceMedium: "Media",
  summaryConfidenceLow: "Baja",
  summaryConfidenceUndetermined: "Sin determinar",
  summaryEvidence: "Evidencia",
  summaryNoEvidence: "No se identificó ningún artículo de respaldo.",
  profileArticle: "Artículo de perfil",
  profileArticleUnavailableTitle: "No hay artículo de perfil publicado",
  profileArticleUnavailableDescription:
    "Esta persona aún no ha publicado una página personal.",
  profileNotFound: "Página personal no encontrada",
  commentsTitle: "Comentarios",
  commentsDescription:
    "Comenta el artículo completo o selecciona un pasaje para dejar un comentario anclado.",
  commentsEmpty: "Aún no hay comentarios. Inicia una conversación reflexiva.",
  writeComment: "Escribir un comentario",
  commentPlaceholder: "Participa en la conversación…",
  postComment: "Publicar comentario",
  signInToComment: "Inicia sesión para comentar",
  commentOnSelection: "Comentar la selección",
  selectedText: "Texto seleccionado",
  clearSelectionComment: "Comentar el artículo completo",
  inlineComment: "Comentario del pasaje",
  fullComment: "Comentario del artículo",
  commentPosted: "Comentario publicado",
  requestFailed: "CTX no pudo completar esta solicitud",
}

export const copy: Record<Locale, Copy> = {
  "zh-CN": chineseCopy,
  "en-US": englishCopy,
  "ja-JP": japaneseCopy,
  "es-ES": spanishCopy,
}

export function initialLocale(): Locale {
  const savedLocale = window.localStorage.getItem(storageKey)
  // COMPATIBILITY: CTX v0.1.0-7 persisted the shorter zh/en locale codes.
  // The first visit rewrites them as canonical BCP 47 tags. Remove after all
  // supported persisted browser state has had a release cycle to migrate.
  if (savedLocale === "zh") return "zh-CN"
  if (savedLocale === "en") return "en-US"
  if (savedLocale && Object.hasOwn(copy, savedLocale))
    return savedLocale as Locale
  const browserLanguage = window.navigator.language.toLowerCase()
  if (browserLanguage.startsWith("ja")) return "ja-JP"
  if (browserLanguage.startsWith("es")) return "es-ES"
  if (browserLanguage.startsWith("zh")) return "zh-CN"
  return "en-US"
}

type I18n = {
  locale: Locale
  setLocale: Dispatch<SetStateAction<Locale>>
  t: (key: TranslationKey) => string
}

const I18nContext = createContext<I18n | null>(null)

export function I18nProvider({ children }: { children: ReactNode }) {
  const [locale, setLocale] = useState<Locale>(initialLocale)

  useEffect(() => {
    window.localStorage.setItem(storageKey, locale)
    document.documentElement.lang = locale
  }, [locale])

  return (
    <I18nContext.Provider
      value={{ locale, setLocale, t: (key) => copy[locale][key] }}
    >
      {children}
    </I18nContext.Provider>
  )
}

export function useI18n() {
  const i18n = useContext(I18nContext)
  if (!i18n) throw new Error("useI18n must be used within I18nProvider")
  return i18n
}
