...
parameters="$(node -e 'const [install,run]=process.argv.slice(1); process.stdout.write(JSON.stringify({commands:[install,run]}))' "printf '%s' '$script_base64' | base64 -d > /tmp/ctx-deploy.sh" "bash /tmp/ctx-deploy.sh $quoted_tag $quoted_archive $quoted_checksum")"
command_id="$(aws ssm send-command --instance-ids "$instance_id" --document-name AWS-RunShellScript --comment "Deploy CTX $tag" --parameters "$parameters" --query 'Command.CommandId' --output text)"
